...
- **Loop engineering**: one agentic node — one agent, one goal, a verifier, a stop condition.
- **Graph engineering**: the topology around those loops — which nodes exist, which transitions are legal, what state crosses each edge.
